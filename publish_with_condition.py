"""
Publish a single item with Card Condition field.
"""
...
def create_and_publish():
    """Create and publish with Card Condition."""
    client = eBayAPIClient()
    
    print("=" * 80)
    print("Creating Single Item with Card Condition")
    print("=" * 80)
    print()
    
    unique_id = str(uuid.uuid4())[:8].upper()
    sku = f"CARD_TEST_{unique_id}"
    
    print(f"SKU: {sku}")
    print()
    
    # Step 1: Create inventory item with conditionDescriptors
    print("Step 1: Creating inventory item with Card Condition...")
    
    item_data = {
        "sku": sku,
        "product": {
            "title": "Test Trading Card - Please Delete",
            "description": "Test trading card listing created via API. Please delete after testing.",
            "aspects": {
                "Card Name": ["Test Card"],
                "Card Number": ["1"],
                "Sport": ["Basketball"],
