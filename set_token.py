"""Helper script to set up eBay token."""
import json
import os

print("=" * 70)
print("eBay Token Setup")
print("=" * 70)
print()

# The token you provided
token_string = "v^1.1#i^1#r^0#f^0#I^3#p^3#t^"

print("This looks like a User Token (not an OAuth token).")
print()
print("You have two options:")
print()
print("Option 1: Use as User Token (simpler)")
print("  - Add this to your .env file:")
print("  - EBAY_PRODUCTION_TOKEN=" + token_string[:50] + "...")
print("  - USE_OAUTH=false")
print()
print("Option 2: Use OAuth (recommended, auto-refreshes)")
print("  - Run: python ebay_bot.py --login")
print("  - This will get you a refresh token that auto-refreshes")
print()

choice = input("Which option? (1 for User Token, 2 for OAuth login): ").strip()

if choice == "1":
    # Update .env file
    env_file = ".env"
    env_content = ""
    
    if os.path.exists(env_file):
        with open(env_file, 'r') as f:
            env_content = f.read()
    
    # Update or add token
    if "EBAY_PRODUCTION_TOKEN=" in env_content:
        # Replace existing
        import re
        env_content = re.sub(r'EBAY_PRODUCTION_TOKEN=.*', f'EBAY_PRODUCTION_TOKEN={token_string}', env_content)
    else:
        env_content += f"\nEBAY_PRODUCTION_TOKEN={token_string}\n"
    
    # Set USE_OAUTH to false
    if "USE_OAUTH=" in env_content:
        env_content = re.sub(r'USE_OAUTH=.*', 'USE_OAUTH=false', env_content)
    else:
        env_content += "USE_OAUTH=false\n"
    
    with open(env_file, 'w') as f:
        f.write(env_content)
    
    print()
    print("✅ Token saved to .env file!")
    print("   Restart the app and policies should load.")
    
elif choice == "2":
    print()
    print("Running OAuth login...")
    print("This will open a browser window for you to authorize.")
    import subprocess
    subprocess.run(["python", "ebay_bot.py", "--login"])
else:
    print("Invalid choice. Exiting.")
